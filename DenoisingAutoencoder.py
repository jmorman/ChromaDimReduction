...
class DenoisingAutoencoder:
    ...
    def __init__(self, input_data, hidden_size, noise_level):
    ...
    def noisify(self, input_vec):
        noisy_vec = []
        for comp in input_vec:
            if(random.random() >= self._nl):
                noisy_vec.append(comp)
            else:
                noisy_vec.append(0)
    ...
    def train(self, num_epochs=50000, verbose=False, blr=1, delta=1e-6):
        ...
        for i in range(num_epochs+1):
            ...
            for input_vector in self._indata:
                ...
                noisy_vector = self.noisify(input_vector)
                
                #Forward propagation: Feeding noisy_vector in
                hidden_layer = self.encode_input(noisy_vector)
                reconstruction = self.decode_hidden(hidden_layer)

                #Backward propagation: Comparing against input_vector, not noisy_vector
    ...
    def save_params(self):
        weights_filename = str(self._k) + 'x' + str(self._m) + 'weights' + str(self._nl) + 'noise.p'
        b_filename = str(self._k) + 'x' + str(self._m) + 'b' + str(self._nl) + 'noise.p'
        c_filename = str(self._k) + 'x' + str(self._m) + 'c' + str(self._nl) + 'noise.p'
    ...
    def load_params(self):
        ...
        c_file = open(c_filename, 'rb')

        self._weights = pickle.load(weights_file)
        self._b = pickle.load(b_file)
        self._c = pickle.load(c_file)
